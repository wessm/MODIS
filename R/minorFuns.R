...
filesUrl <- function(url)
{
    ...
    ## NTSG method; if not used, connection breakdowns are likely to occur
      ...
      jnk <- capture.output(
        download.file(url,destfile = dest, quiet=FALSE, method = 'wget', extra=paste0('--user anja.klisch --password AnjaKlisch1 --load-cookies ~/.cookies.txt --save-cookies ~/.cookies.txt --keep-session-cookie --no-check-certificate'))
      )
